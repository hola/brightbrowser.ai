...
Signing in to login forms using BrowserAI requires successful completion of KYC procedures before any activity can be performed.
...
<Steps>
  ...
  <Step title="Submit OTP for Verification">
    ...
    - Submit the OTP as a new instruction for the same task by using the [`/instructions` API endpoint](https://docs.browser.ai/api-reference/endpoint/instructions).
