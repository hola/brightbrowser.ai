...
sidebarTitle: Custom
description: "BrightBrowser.ai enhances standard CDP functions with custom functions for better control, stealth, and efficiency in web automation. Developers can bypass anti-bot systems, optimize scraping, and fine-tune browser behavior with ease."
---

## Captcha Solver

BrightBrowser.ai’s **built-in CAPTCHA solver automatically handles all CAPTCHAs**, keeping your automation uninterrupted. Use custom CDP functions to monitor and fine-tune the solving process directly in your code.
...
<Note>
  Once a CAPTCHA is solved, BrightBrowser.ai automatically submits the form if required.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Custom commands for CAPTCHA status">
    Use the following commands to identify specific stages in the CAPTCHA-solving process and debug more effectively.:

    |                         |                                                                      |
    | ----------------------- | -------------------------------------------------------------------- |
    | `Captcha.detected`      | BrightBrowser.ai has encountered a CAPTCHA and has begun to solve it |
    | `Captcha.solveFinished` | BrightBrowser.ai successfully solved the CAPTCHA                     |
    | `Captcha.solveFailed`   | BrightBrowser.ai failed in solving the CAPTCHA                       |
    | `Captcha.waitForSolve`  | BrightBrowser.ai waits for CAPTCHA solver to finish                  |
    ...
    **Examples**
    ...
    <Tabs>
      ...
      <Tab title="Synchronous">
        ...
        The `Captcha.waitForSolve` command waits for BrightBrowser.ai's CAPTCHA solver to finish.
...
<AccordionGroup>
  ...
  <Accordion title="Code examples">
    * Replace placeholder values `USER:PASS` with your valid Scraping Browser credentials.
    ...
    * Replace `secret` with the actual password for the certificate.
          ...
          <CodeGroup>
            ...
            async function scrape(url = TARGET_URL, file = CERT_FILE, pass = CERT_PASS) {
              if (AUTH == 'USER:PASS') {
                throw new Error(`Provide BrightBrowser.ai credentials in AUTH`
            ...
            def scrape(url=TARGET_URL, file=CERT_FILE, pswd=CERT_PASS):
                if AUTH == 'USER:PASS':
                raise Exception('Provide BrightBrowser.ai credentials in AUTH '
                                'environment variable or update the script.')
