...
sidebarTitle: "Custom"
description: "infraBrowser.ai enhances standard CDP functions with custom functions for better control, stealth, and efficiency in web automation. Developers can bypass anti-bot systems, optimize scraping, and fine-tune browser behavior with ease."
---

## Captcha Solver

infraBrowser.ai’s **built-in CAPTCHA solver automatically handles all CAPTCHAs**, keeping your automation uninterrupted. Use custom CDP functions to monitor and fine-tune the solving process directly in your code.
...
<Note>
  Once a CAPTCHA is solved, infraBrowser.ai automatically submits the form if required.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Custom functions for CAPTCHA status">
    ...
    |                         |                                                                     |
    | ----------------------- | ------------------------------------------------------------------- |
    | `Captcha.detected`      | infraBrowser.ai has encountered a CAPTCHA and has begun to solve it |
    | `Captcha.solveFinished` | infraBrowser.ai successfully solved the CAPTCHA                     |
    | `Captcha.solveFailed`   | infraBrowser.ai failed in solving the CAPTCHA                       |
    | `Captcha.waitForSolve`  | infraBrowser.ai waits for CAPTCHA solver to finish                  |
    ...
    **Examples**
    ...
    <Tabs>
      ...
      <Tab title="Synchronous">
        ...
        The `Captcha.waitForSolve` command waits for infraBrowser.ai's CAPTCHA solver to finish.
...
<AccordionGroup>
  ...
  <Accordion title="Code examples">
    * Replace placeholder values `USER:PASS` with your valid infraBrowser.ai credentials.
    ...
    * Replace `secret` with the actual password for the certificate.
          ...
          <CodeGroup>
            ...
            async function scrape(url = TARGET_URL, file = CERT_FILE, pass = CERT_PASS) {
              if (AUTH == 'USER:PASS') {
                throw new Error(`Provide infraBrowser.ai credentials in AUTH`
            ...
            def scrape(url=TARGET_URL, file=CERT_FILE, pswd=CERT_PASS):
                if AUTH == 'USER:PASS':
                raise Exception('Provide infraBrowser.ai credentials in AUTH '
                                'environment variable or update the script.')
